...
class ChatListLockScreen extends StatelessWidget {
  const ChatListLockScreen({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ChatController>(initState: (state) {
      ...
      controller.chatLockPagingController = PagingController(firstPageKey: 1);
      controller.chatLockPagingController
          .addPageRequestListener((pageKey) async {
        await controller.postChatLockFriends(pageKey);
      });
    }, builder: (controller) {
      return Scaffold(
                  ...
                  children: [
                    Expanded(
                      child: CustomTextFormField(
                        controller: controller.serchChatLockController,
                        hintText: 'search'.tr,
                        ...
                        onChanged: (value) {
                          debouncer.run(() {
                            Future.sync(
                              () {
                                return controller.chatLockPagingController
                                    .refresh();
                              },
                            );
                          });
                ...
                Expanded(
                  child: RefreshIndicator(
                    onRefresh: () => Future.sync(
                      () => controller.chatLockPagingController.refresh(),
                    ),
                     color: ColorsValue.appColor,
                    child: PagedListView<int, FriendsListDatum>(
                      pagingController: controller.chatLockPagingController,
                      builderDelegate:
                          ...
                          return GestureDetector(
                            onLongPressStart: (details) {
                              if (Get.find<HomeScreenController>()
                                      .selectedChateData ==
                                  null) {
                                ChatScreenUtility.pinUnpinSingaleChatLock(
